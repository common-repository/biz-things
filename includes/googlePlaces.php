...
function bizthings_search($location) {
	$apiKey = get_option( 'biz_things_google_places_id', '' );
	//thanks bartdyer for the simple solution
	$location=ereg_replace("&", "and", $location);
  ...
  function bizthings_placeDetails($placeId) {
	  if (!NULL==$placeId) {
	  $apiKey = get_option( 'biz_things_google_places_id', '' );
	  $ch = curl_init();
